...
class KuduContext(val kuduMaster: String,
  ...
  /**
   ...
   def kuduRDD(sc: SparkContext,
              tableName: String): RDD[Row] = {
    this.kuduRDD(sc, tableName, Nil,Array())
  }
  def kuduRDD(sc: SparkContext,
              tableName: String,
              predicates: Array[KuduPredicate]): RDD[Row] = {
    this.kuduRDD(sc, tableName, Nil,predicates)
  }
  def kuduRDD(sc: SparkContext,
              tableName: String,
              columnProjection: Seq[String]): RDD[Row] = {
    this.kuduRDD(sc, tableName, columnProjection,Array())
  }
  def kuduRDD(sc: SparkContext,
              tableName: String,
              columnProjection: Seq[String],
              predicates: Array[KuduPredicate]): RDD[Row] = {
    new KuduRDD(this, 1024*1024*20, columnProjection.toArray, predicates,
